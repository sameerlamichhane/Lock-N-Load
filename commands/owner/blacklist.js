...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		const guide = new MessageEmbed()
			...
			.addField(
				`<a:arrow_right:1133392521297399960> ${prefix}bl add <user> all`,
				`Add a user to blacklisted users for all servers.`
			)
			.addField(
				`<a:arrow_right:1133392521297399960> ${prefix}bl remove <user> all`,
				`Remove a user from blacklisted users from all servers.`
			)
			.addField(
				`<a:arrow_right:1133392521297399960> ${prefix}bl show`,
				`Shows all the users in blacklisted database.`
			)
			.addField(
				`<a:arrow_right:1133392521297399960> ${prefix}bl reset`,
